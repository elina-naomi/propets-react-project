...
const SimplePost = (props) => {
    ...
    function cutText(text) {
        ...
        let temp = text.split(' ');
        if(temp.length>size) {
            temp = temp.slice(0,size).join(' ');
            return <Fragment>{temp} <span onClick={()=> {setIsMore(false)}}
                                          className={`${styles.more}`}> {isMore? '...more':''}</span>
            </Fragment>;
        } else return text;
            ...
            <div className={`container-sm`}>
                <div className='row'>
                    <div className='col-1 p-0'>
                        <img className={`${profileStyles.avatar}`} src={props.avatar} alt='profile'/>
                    </div>
                    <div className='col-9'>
                        <p className={`${styles.authorName}`}>{props.userName}</p>
                        <p className={`${styles.time}`}>{props.datePost}</p>
                        {props.images? <img className={`${styles.postImage} mt-2`} width='100%' src={props.images} alt='post'/> : null}
                        <p className={`${styles.text} mt-2`}>
                            {isMore? cutText(props.text) : props.text}
                        </p>
